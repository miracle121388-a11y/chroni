...
- 新增本地语音助手：支持查询与总结日程、创建/完成/改期任务、运行 Agent、启动专注计时和打开工作区；全局快捷键默认为 `Ctrl/Command+Shift+Space`。
- 新增本地 Whisper 转写、语音文字执行记录、系统语音播报和后台专注结束提醒；麦克风原始音频仅在内存中处理，所有写操作使用短时预览令牌并由用户确认。

### Changed

- 发布页同步展示本地语音助手、确认式写操作与语音隐私边界；移除逐帧滚动样式计算和超长粘性场景，改为一次性入场与按需分区渐显，并为低性能设备和省流量模式自动降级动效。
- 产品页首屏、下载区、发布状态和响应式布局进一步优化，突出无需账号、无需 API Key 与本地优先能力，并修复桌面宽屏下首屏文字和产品演示重叠的问题。
- 托管网关在代码层固定使用 `deepseek-v4-flash`，遗留的 Zeabur `DEEPSEEK_MODEL` 不再能够把公共客户端静默切换到 Pro；新增下载、仓库流量、网关调用和模型费用的运营数据说明。
- Windows 与 macOS 安装包声明麦克风用途并携带本地 ONNX 推理运行时；语音模型在首次使用时下载到本机缓存，不嵌入安装包。

### Fixed

- 产品页不再依赖 GitHub API 成功后才能直达安装包；Windows 安装版、Windows 便携版与 macOS DMG 均使用跨版本稳定下载地址，接口限流或不可用时也不会跳转 Release 列表。
- 发布工作流自动为三个公开安装包生成稳定别名，并将其纳入校验和与构建来源证明。
