...
2. 在“日程”中快速添加，或下载并拖入[课程作业示例](./examples/demo/01-course-assignment.txt)。也可以新建包含下面内容的 TXT：
...
Chroni 的桌宠是一只安静的蓝色毛绒伙伴，也是 Deadline Agent 留在桌面上的轻量入口。它不会用持续弹窗打断工作，而是把“正在读取、正在规划、需要注意、已经完成”等系统状态变成自然的动作与短气泡。即使控制中心关闭，用户仍能从桌面上感知任务进度，并在需要时立即回到日程。
...
- **动作即状态**：Agent 处理材料时学习，截止时间临近、任务逾期或需要补充信息时主动唤醒，完成任务后播放庆祝动作。
...
当前版本提供“本地规则”“Chroni 内测”和“自定义 API”三种实际工作方式。受邀测试者只需填写可撤销的内测访问码；DeepSeek 主密钥保存在 Zeabur 网关，永远不会写入桌面安装包。三种方式的适用范围、费用与安全边界见[模型使用方式](./docs/user/model-modes.md)，网关部署与运维见[内测 LLM 网关](./docs/llm-gateway.md)。

### 控制中心配置（推荐）

1. 从托盘打开“控制中心”，进入“偏好”。
2. 展开“高级 → 智能模型服务”。
3. 受邀测试者选择“Chroni 内测”，填写邀请中提供的访问码。
...
- Chroni 内测服务使用维护者设置的受控额度，可能触发分钟、并发或每日限额；长期使用可切换为自定义 API。
...
| 验证层级 | 当前基线 |
| --- | --- |
| 自动化测试 | 229 项 Node 测试定义，覆盖文件接收、中文相对时间、无效模型输出回退、主动追问、TaskPlan、Deadline Agent、Memory、窗口交互、API 安全与打包配置；最新本地结果与已知阻塞见[产品化审计](./docs/productization-roadmap.md)。 |
