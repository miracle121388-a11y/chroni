...
> [!IMPORTANT]
> Chroni 是“大模型 API 应用 / Agent 系统”课程项目的完整开源实现。当前公开安装包用于课程验收与测试，可能尚未配置 Windows 代码签名或 macOS Developer ID 公证，因此系统可能显示 SmartScreen / Gatekeeper 提示；请只从本仓库 Releases 下载，并核对 SHA-256 校验和。
...
| 课程要求 | Chroni 的对应实现 |
| --- | --- |
| 调用大模型 API | 接入 OpenAI-compatible Chat Completions；默认示例使用 DeepSeek，参与语义抽取、追问表达、任务拆解和可选的每日规划。 |
| 构建大模型或 Agent 应用 | Deadline Agent 具备工具调用、Memory、结构化 Trace、`Observe -> Plan -> Act -> Verify` 循环与失败回退，不是单次 Prompt 包装。 |
| 确保实用性 | 从通知、文档、表格、日历和截图生成可执行的 DDL、TaskPlan、每日时间块与桌面提醒。 |
| 在 GitHub 开源 | 源码、测试、设计文档、安装包、校验和、构建证明与贡献指南均在本仓库公开。 |
...
3. 预期结果是直接得到标题“课程项目”和本地时间 20:00 的截止任务；标题和截止时间都已明确，因此不应再询问“任务叫什么”或“何时截止”。这一行为有自动化回归测试覆盖。
...
这条路径覆盖 `输入 -> 理解 -> 校验 -> 必要时追问 -> 拆解 -> 激活 -> 排程 -> 提醒 -> 完成 / 重排`，可以直接用于课程演示与验收。
...
> [!WARNING]
> 未签名的课程测试包可能触发 Windows SmartScreen 或 macOS Gatekeeper。不要关闭系统全局安全机制；请先核对仓库、版本和校验和，再只对已验证的 Chroni 文件执行系统提供的“仍要运行 / 仍要打开”。正式公开分发应配置代码签名与 macOS 公证。

配置有效平台签名的安装包会在后台检查 GitHub Releases；新版本下载完成后，“运行状态”页面会出现“重启并安装”，应用不会在工作过程中突然重启。未签名课程测试包仍可从托盘发起检查，但 macOS 可能拒绝自动更新，此时请前往 Releases 手动下载安装。
...
- Chroni 负责规划、提醒和记录，不会代替用户上传作业、发送邮件或宣布任务已经完成；最终 DDL 与完成状态由用户掌握。
- 当前正式支持并发布 Windows 10/11 x64 与 macOS 12+ Universal 安装包；Linux 可用于开发和 CI，但暂不作为公开安装包承诺。
- 课程测试包可能未签名或未公证，系统安全提示不等于文件损坏；请通过 Release 校验和与构建证明确认来源。
