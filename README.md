...
<p align="center">
  <strong>把课程材料变成今天能执行、晚上能复盘的本地学习 Agent。</strong>
</p>

<p align="center">
  Local-first learning execution agent for Windows and macOS.<br>
  拖入通知、文档或图片，Chroni 会依据原文完成智能整理、任务拆解与日程安排；<br>
  只在真正阻断执行时追问，并通过每日回顾与桌宠提醒陪伴计划持续落地。
</p>

<p align="center">
  <strong>本地语音助手 · 材料理解 · 今日计划 · 每日回顾 · 桌宠协作</strong>
</p>
...
| 本地语音助手 | 按全局快捷键直接询问日程、创建或完成任务、调整时间、运行 Agent 与启动专注计时；语音在本机转写，写操作确认后才执行。 |
...
| 学习执行 Agent | 综合学业语境、截止、进度、剩余工作、历史未完成、证据反馈和每日容量，运行 `Ground → Plan → Act → Verify → Adapt` 闭环。 |
| 每日执行视图 | 提供日、多日、周、月视图与待安排区；Agent 避开已有课程/会议，在可用时间缝隙排程，任务仍可拖拽、缩放和重新安排。 |
| 每日回顾 | 独立栏目按日期整理活动轨迹，并对比前后 7 天完成率、执行效率、计划未完成率和节奏平衡；结果用于下一轮容量与干预调整。 |
...
6. 完成一个阶段后，登记产出文件或文字证据，并提交一次阶段检查点；一天结束时打开“每日回顾”，查看活动轨迹并保存完成情况、待延续事项与个人记录。

这条体验路径不要求 API Key。完整的安装、桌宠入口、统一导入流程、Demo 材料和退出方式见 [Chroni 3 分钟快速开始](./docs/user/quick-start.md)。

## 本地语音助手

Chroni 的第一版语音助手不是一个只能聊天的悬浮麦克风，而是连接日程、任务、Agent 和桌宠的受控操作入口。控制中心右下角的语音按钮与全局快捷键均可唤起；首次使用会下载本地 Whisper 语音模型，完成后模型缓存到应用数据目录，之后可离线转写。

| 你可以说 | Chroni 会做什么 |
| --- | --- |
| “我今天有什么安排” | 直接朗读并展示今日日程，不产生修改。 |
| “明天下午三点写项目报告，安排一个小时” | 先展示日期、时间和任务名称，确认后写入每日任务。 |
| “完成机器学习作业” | 匹配现有任务，确认后更新完成状态。 |
| “把答辩准备改到晚上八点” | 匹配任务并预览新时间，确认后调整。 |
| “帮我安排今天” | 预览 Agent 规划动作，确认后运行每日安排。 |
| “专注四十五分钟” | 确认后启动后台计时，结束时由系统通知和桌宠提醒。 |
| “总结今天” | 汇总已完成、未完成和投入情况，并可使用系统语音播报。 |

原始麦克风音频只在内存中处理，不写入磁盘，也不上传。Chroni 最多在本机保留最近 100 条语音文字与执行摘要，便于解释助手刚刚做了什么；关闭“偏好 → 语音播报结果”可保留文字反馈但停止朗读。无法匹配到受控指令的语音会进入现有智能整理流程，因此在启用托管或自定义 LLM 时，转写后的文字可能按现有模型设置发送，详见[隐私说明](./docs/user/privacy.md)。
...
输入、文件接收与 Agent 安排合并在一个低打扰工作区。明确材料会直接建立任务并更新今日安排；只有标题或截止时间确实无法可靠判断时，才逐项请求确认。

![Chroni 智能整理工作区，展示统一输入、文件导入、风险和已整理任务](./docs/store/assets/screenshots/zh-CN/03-smart-organize.png)

_底层仍运行完整学习执行 Agent，界面只保留添加材料、更新安排、阻塞确认和结果检查。_

### 每日回顾

![Chroni 每日回顾栏目，展示回顾日历、历史记录、活动轨迹、完成指标与个人整理](./docs/store/assets/screenshots/zh-CN/04-daily-review.png)

_每日回顾是独立工作区：按自然日保存活动、自动总结和个人记录，历史日期可随时回看，未完成事项会明确承接到后续计划。_
...
- 没有文本层的扫描 PDF 会逐页渲染再 OCR，最多处理 `60` 页；OCR 可靠性阈值为 `55`。
...
| 验证层级 | 当前基线 |
| --- | --- |
| 自动化测试 | 当前 Desktop 285 项：284 通过、0 失败、1 跳过；Gateway 6 项全部通过。覆盖本地语音转写与受控命令、文件接收、中文相对时间、无效模型输出回退、低打扰追问、TaskPlan、Learning Mission、语义优先级、证据与检查点、学习执行 Agent、14 日回顾趋势、Memory、日历并发排布、窗口交互、API 安全与打包配置。 |
