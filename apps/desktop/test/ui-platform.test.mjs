...
test("renderer bundles local cross-platform fonts under the production CSP", async () => {
  ...
  assert.equal(packageJson.devDependencies["@fontsource-variable/inter"], "5.2.8");
  assert.equal(packageJson.devDependencies["@fontsource-variable/noto-sans-sc"], "5.2.10");
  assert.match(stylesSource, /--font-ui:\s*"Inter Variable",\s*"Noto Sans SC Variable",\s*sans-serif;/);
  assert.match(stylesSource, /:root\s*\{[\s\S]*?font-family:\s*var\(--font-ui\);/);
  assert.doesNotMatch(stylesSource, /Segoe UI|PingFang SC|Microsoft YaHei/);
