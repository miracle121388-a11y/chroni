      ...
      <aside className={`daily-editor color-${draft.color}`} role="dialog" aria-modal="true" aria-labelledby="daily-editor-title" aria-describedby={recurringSeries ? "daily-series-notice" : undefined}>
        ...
        <form onSubmit={(event) => void save(event)}>
          ...
          <div className="daily-editor-row">
            ...
            {draft.scheduled && <label>日期<UiDateTimeField required type="date" value={draft.date} disabled={!!busy} onChange={(date) => setDraft({ ...draft, date })} /></label>}
          </div>
          {draft.scheduled && !draft.allDay && <div className="daily-editor-row"><label>开始<UiDateTimeField required type="time" value={draft.start} disabled={!!busy} onChange={(start) => setDraft({ ...draft, start })} /></label><label>结束<UiDateTimeField required type="time" value={draft.end} disabled={!!busy} onChange={(end) => setDraft({ ...draft, end })} /></label></div>}
          {draft.scheduled && <div className="daily-editor-row"><label>重复<select value={draft.recurrence} disabled={!!busy} onChange={(event) => setDraft({ ...draft, recurrence: event.target.value as DailyTaskRecurrence })}><option value="none">不重复</option><option value="daily">每天</option><option value="weekdays">工作日</option><option value="weekly">每周</option></select></label><label className="daily-all-day-toggle"><span>全天</span><input className="ui-checkbox" type="checkbox" checked={draft.allDay} disabled={!!busy} onChange={(event) => setDraft({ ...draft, allDay: event.target.checked })} /></label></div>}
          {draft.scheduled && draft.recurrence !== "none" && <div className="daily-editor-row"><label>重复结束日期（可选）<UiDateTimeField type="date" min={draft.date} value={draft.recurrenceEndsAt} disabled={!!busy} onChange={(recurrenceEndsAt) => setDraft({ ...draft, recurrenceEndsAt })} /></label></div>}
