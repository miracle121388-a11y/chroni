...
| Oversized input | Renderer limits each file to 18 MiB; API body is capped at 32 MiB; path reads are descriptor-bound and length-bounded; parsers have text/document limits; scanned-PDF OCR is capped at 60 pages and rendered one page at a time; OOXML preflight caps 2,000 entries and 64 MiB expanded size. | `filesFromFileList`, `MAX_API_BODY_BYTES`, intake safety tests | Pathological parser CPU still needs worker isolation and cancellation. |
