...
Chroni 的语音识别在本机运行。第一次点击语音按钮时，应用需要联网下载 Whisper 模型并缓存到本机，因此会显示“正在准备本地语音模型”；后续启动会复用缓存。模型准备完成并显示“可以说话”后再录音，单次语音最长 30 秒。

如果一直无法开始：

1. 在 Windows“设置 -> 隐私和安全性 -> 麦克风”或 macOS“系统设置 -> 隐私与安全性 -> 麦克风”中允许 Chroni 使用麦克风。
2. 确认首次下载时可以访问 Hugging Face；代理、防火墙或断网会阻止模型准备。
3. 从“偏好”确认语音助手已开启，再从“运行状态”刷新检查。
4. 完全退出 Chroni 后重启。仍失败时按[故障排查](./troubleshooting.md)提交错误信息，不要上传真实语音内容。
...
可以。新安装默认使用不需要 Key 的本地规则；需要模型抽取、主动追问、TaskPlan 和 Agent 规划时，可在阅读数据发送说明并明确同意后开启 Chroni 智能服务，仍不要求 API Key、访问码或账号。公共服务有按来源网络和全局额度设置的公平使用限制；暂时不可用或达到额度时会自动回退本地规则。
